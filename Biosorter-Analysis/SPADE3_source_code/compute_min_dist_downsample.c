...
/* calculate the L1 distance between each item in prhs[0] and its nearest neighbor in prhs[1]    */
    ...
    for(j=0;j<n;j++){
        ...
        *NN_index=0;                    /* for the j'th new point, the NN and min_dist has never been assigned yet */
        ref_data_i = ref_data;
        while(i<N && *NN_index==0){     /* find the first point in the ref set that is not identical to the new point, use this point to initialize NN and min_dist */
            dist_tmp=0;
            for(d=0;d<dim;d++)
        ...
        while(i<N){                     /* visit the remaining points in the ref set, and see whether there is anyone closer */
            dist_tmp=0;
            for(d=0;d<dim;d++)
                dist_tmp += fabs(new_data_j[d] - *(ref_data_i++));
            if(dist_tmp<*min_dist && dist_tmp!=0)
            {
                *min_dist = dist_tmp;
                *NN_index=i+1;
            }
