...
MSG(:,1)=mean(MS(:,1:7),2);
MSG(:,5)=mean(MS(:,8:14),2);
MSG(:,5)=mean(MS(:,15:21),2);
MSG(:,5)=mean(MS(:,21:28),2);
...
[~,~]=STOCSY_BS(ppmMS_g(200*ind-99),Jointdata,Jointppm,bsize,(size(MS_g,2)+1),UL);
title(['STOCSY, target: ',glycans{ind},' (',num2str(ind),')'])
saveas(gcf,[num2str(ind),'New.fig']);
close
end