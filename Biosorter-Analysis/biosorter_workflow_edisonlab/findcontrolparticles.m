function [controlparticles, controlparticles_parameters_list, mean_controlparticles, stds_controlparticles, intersectt] = findcontrolparticles...
    (numericparameters, numericparameters_list, sorter_settings, makeplot)
% finds control particles
% the paramters to find control particles depend on the settings
% used in the acquisition. If the PMT values are very low, the 
% thresholds for the control particles need to be adjusted.
% thresholds can be modified in line 64
...
% values to detect control particles can be modified here
...
% length(controlparticles) would be the number of files 
...
% this gives the location of each event in each file that meets the control particle criteria
...
% extract controlparticles data into new cell array
...
% ploting - generates plots of control particle data

if exist('makeplot')

sorter_settings = sorter_settings(2:end,:);
extdetectorpower = sorter_settings(:,21);
extdetectorpower = char(extdetectorpower);
extdetectorpower = str2num(extdetectorpower);
...
subplot(3,2,2)
mean_PHGreen_cp = cell2mat(mean_controlparticles(:,8));
std_PHGreen_cp = cell2mat(stds_controlparticles(:,8));
...
subplot(3,2,3)
mean_PHYellow_cp = cell2mat(mean_controlparticles(:,10));
std_PHYellow_cp = cell2mat(stds_controlparticles(:,10));
...
subplot(3,2,4)
mean_PHRed_cp = cell2mat(mean_controlparticles(:,12));
std_PHRed_cp = cell2mat(stds_controlparticles(:,12));
