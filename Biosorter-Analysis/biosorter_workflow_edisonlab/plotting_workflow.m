%% Binning workflow
% Francesca V. Ponce/MBC
% July 2016
% PROJECT ID HERE
%% Be sure that workspace variables from wormstorter_workflow are loaded before running this script
% Purpose must be to choose which data, under which normalization parameters,
% from which samples are displayed at once.
%% Key for tracking value processing. same pattern for all other parameters
% ext = cell array raw extinction values from data files with gaps from control
% particles
% Ext = matrix with empty spaces erased
% nExt = normalized extinction values (divided by the mean control particle
% extinction from relevant sample file)
% lnExt = log of normalized extinction values
%% 
%create new matrix of all data minus all recognized control particles
%specified by intersectt index
