...
for(inetwork in nnetworks_seq){#different random network
  for(ncondi in ncondiseq){##different conditions
    ...
    peakvec=listres[[paste0(inetwork,"_",ncondi)]][["peakvec"]]
    ...
    for(clust in setdiff(unique(clust_real_obs),0)){
      ...
      countoverlap=table(clust_est[mem_real])
      big_overlap_size=countoverlap[names(sort(countoverlap,decreasing=TRUE)[1])]
...
save(listeval,file=paste0("./","net.CausalKinetiX.all.clusterres.bigpartial.mulpeak.new.RData"))
...
ggsave(paste0("multipeak_partial_overlapratio_boxplot.pdf"),plot=p)
# plot with raw data
...
p<-ggplot(data=clust_overlap_frame,aes(x=condi,y=val,color=clust,group=clust))+
  ...
  labs(title=expression(paste("match ratio",)),x="condition number",y="match ratio")+
  theme(axis.text=element_text(size=10))
ggsave(paste0("multipeak_partial_overlapratio_raw.pdf"),plot=p)
# wilcox t test
stattab=c()
...
# accuracy and recovery (in terms of each compounds)
...
for(inetwork in nnetworks_seq){#different random network
  for(ncondi in ncondiseq){##different conditions
    ...
    peakvec=temp_list[["peakvec"]]
    # there will be edges unobervable so should not be counted as ground truth
    obs_nodes=unique(unlist(edgetab_est))
    obs_compd=unique(peakvec[obs_nodes])
    edgetab_real=edgetab_real[edgetab_real[,"from"]%in%obs_compd & edgetab_real[,"to"]%in%obs_compd,]
    # convert peak to compound in estimation
    edgetab_est_compd=cbind(peakvec[edgetab_est[,1]],peakvec[edgetab_est[,2]])
    edgetab_est_compd=t(apply(edgetab_est_compd,1,sort))
    edgetab_est_compd=unique(edgetab_est_compd)
    corr_count=sum(apply(edgetab_real,1,function(x){
      loc=which((edgetab_est_compd[,1]==x[1]&edgetab_est_compd[,2]==x[2])|(edgetab_est_compd[,1]==x[2]&edgetab_est_compd[,2]==x[1]))
      length(loc)>0
    }))
    liststat[[paste0(inetwork,"_",ncondi)]]=c(corr_count/dim(edgetab_real)[1],corr_count/dim(edgetab_est_compd)[1])
  }
}
# stat
...
p<-ggplot(data=summarytab,aes(x=condi,y=mean,color=variable,group=variable))+
  ...
  theme(axis.text=element_text(size=10))
ggsave(paste0("multipeak_partial_prec_recall.pdf"),plot=p)
