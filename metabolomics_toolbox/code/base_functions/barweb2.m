...
% Parameters: width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type
%
% Ex:
% x=normrnd(0,1,2,8);
% s=normrnd(0,.5,2,8);
% h = barweb(x,s,'error_sides',3,'bw_xlabel','Genes','bw_ylabel','Fold Change','groupnames',{'Gene A','Gene B'});
%
...
% Edited by GSS Nov 2013
% Edited by CSC July 2014

if nargin < 2
    error('Must have at least the first two arguments')
end
% Get function arguments
options = struct('width',1,'groupnames',1:size(barvalues,1),'bw_title',[],...
    'bw_xlabel',[],'bw_ylabel',[],'bw_colormap',jet,'gridstatus','none',...
    'bw_legend',[],'error_sides',3,'legend_type', 'plot');
...
if ~isempty(varargin)
    for pair = reshape(varargin,2,[]) % pair is {propName;propValue}
        ...
        m=strcmpi(inpName,optionNames);
        if any(m)
            options.(optionNames{m})=pair{2};
        else
...
for i=1:length(optionNames)
    eval([optionNames{i}, '=options.(optionNames{i});']);
end
