function VisScoresLabel(X,model,components,Y)
...
if length(components)==1
	...
	text(model.scores(k,components(1)),Y(k),num2str(Y(k)),'VerticalAlignment','bottom','HorizontalAlignment','right')
    end
    xlabel(['Component',num2str(components(1)),', ', num2str(model.variance(components(1))*100),' Percent of Variance'])
   
elseif length(components)==2
    %figure, 
    hold on
    for k=1:size(model.scores,1);
        plot(model.scores(k,components(1)),model.scores(k,components(2)),'o','LineWidth',1,'MarkerEdgeColor','k','MarkerFaceColor',cmap(Ycolor(k),:),'MarkerSize',8)
	text(model.scores(k,components(1)),model.scores(k,components(2)),num2str(Y(k)),'VerticalAlignment','bottom','HorizontalAlignment','right')
    end
    xlabel(['Component ',num2str(components(1)),', ', num2str(model.variance(components(1))*100),' Percent of Variance'])
    ylabel(['Component ',num2str(components(2)),', ', num2str(model.variance(components(2))*100),' Percent of Variance'])
...
elseif length(components)==3
    ...
    xlabel(['Component',num2str(components(1)),', ', num2str(model.variance(components(1))*100),' Percent of Variance'])
    ylabel(['Component',num2str(components(2)),', ', num2str(model.variance(components(2))*100),' Percent of Variance'])
    zlabel(['Component',num2str(components(3)),', ', num2str(model.variance(components(3))*100),' Percent of Variance'])
