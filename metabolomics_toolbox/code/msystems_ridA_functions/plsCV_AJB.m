function PLS=plsCV_AJB(Y,matrix,nfold,type,permutations)
...
if size(Y,2) == 1
  if size(stats.W,2) ~= 1
    reg = cumsum((stats.W*diag(YL))');
  else
    reg = (stats.W*YL)';
  end
else
  reg = zeros(size(Y,2)*numcomponents,size(matrix,2));
  reg(1:size(Y,2),:) = YL(:,1)*stats.W(:,1)';
  for i = 2:numcomponents
    reg((i-1)*size(Y,2)+1:i*size(Y,2),:) = YL(:,i)*stats.W(:,i)' + reg((i-2)*size(Y,2)+1:(i-1)*size(Y,2),:);
  end
end
