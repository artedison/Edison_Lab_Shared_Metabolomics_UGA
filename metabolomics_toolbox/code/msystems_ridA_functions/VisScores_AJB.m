function M = VisScores_AJB(X,model,components,varargin)

% Author : Edison lab, modified AJB and GG
% Ver : 1.3 
...
%                   sample 
%              IMPORTANT: List samples in order from lowest to highest
%              according to sample ID number
...
add_col=[];
label_ax=[];
% Find unique values and their indices for the Y-vector
% This finds the unique values within 'Ycolor' and the index vectors 'Upos'
% and 'Upos2' such that Ucomp = Ycolor(Upos) and Ycolor(Upos2) = Ucomp
% Example: 
%          A = [9 2 9 5];
%            [C, ia, ic] = unique(A)
%          C = 1×3    [2,5,9]
%          ia = 3×1   [2;4;1]
%          ic = 4×1   [3;1;3;2]
[Ucomp,Upos,Upos2]=unique(Ycolor);
fmt='%0.2f';
...
elseif length(components)==2
    for k=1:size(model.X_scores,1)
        ax(k)=plot(model.X_scores(k,components(1)),model.X_scores(k,components(2)),'o','LineWidth',1,'MarkerEdgeColor','k','MarkerFaceColor',cmap(Ycolor(k)+1,:),'MarkerSize',8);
        if args.showlabels
            text(model.X_scores(k,components(1)),model.X_scores(k,components(2)),strcat(int2str(args.Y(k)),{'-'},int2str(k)),'fontsize',10,'HorizontalAlignment','left','VerticalAlignment','bottom')
        end
    end
  if iscell(args.showlegend) 
      for l=1:size(Upos,1)
            label_ax=[label_ax,ax(Upos(l))];
            sample_name=[sample_name,args.showlegend(l)];
      end
  end
    xlabel(['Component ',num2str(components(1)),', ', num2str(model.variance(components(1))*100, fmt),'% of Variance'])
    ...
    %plot 95% confidence ellipses
        ...
        for i=1:max(Ygrp)
            D=[model.X_scores(Ygrp==i,components(1)),model.X_scores(Ygrp==i,components(2))];
            mu = mean(D);
    ...
    if any(args.mahalanobis)
        ...
        scores=model.X_scores(:,components);
...
elseif length(components)==3
    for k=1:size(model.X_scores,1);
        plot3(model.X_scores(k,components(1)),model.X_scores(k,components(2)),model.X_scores(k,components(3)),'o','LineWidth',1,'MarkerEdgeColor','k','MarkerFaceColor',cmap(Ycolor(k)+1,:),'MarkerSize',8)
    end
    view(3)
    xlabel(['Component ',num2str(components(1)),', ', num2str(model.variance(components(1))*100, fmt),'% of Variance'])
    ylabel(['Component ',num2str(components(2)),', ', num2str(model.variance(components(2))*100, fmt),'% of Variance'])
...
%  legend([ax(2),ax(11),ax(18),ax(26),ax(27),ax(30)],'WT_M_M  (n=10)','{\itridA}_M_M (n=10)','WT_G_l_y  (n=10)','{\itridA}_G_l_y (n=10)','WT_I_l_e   (n=10)','{\itridA}_I_l_e  (n=10)');
