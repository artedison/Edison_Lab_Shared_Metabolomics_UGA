function HRMAS_nmr_processAndStoreData3(sampleName,fileName,saveData,local)
...
%% PROCESSING
    %% Load the Processed data from ft files
        % Load all the ft files as spectra
            cd(data_directory)
            cd(['NMR/HRMAS/',fileName,'/nmrpipe/ft_1h1d'])
            loadallft();
    ...
    %% Save
        ...
        if strcmp(saveData,'save') || local
            fprintf(['\n\tWriting "',fileName,'_1h1d_data.mat" in \n\t',scripts_directory,'...\n\n'])
            cd(scripts_directory);save([fileName,'_1h1d_data.mat'])
        end
end
