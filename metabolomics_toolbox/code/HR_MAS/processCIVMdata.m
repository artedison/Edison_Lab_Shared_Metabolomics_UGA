                        ...
                        % If reprocessing, copy the existing data for storage:
                            ...
                            if response == 1
                                % Move the files to another directory
                                    cd(specList.paths.scripts)
                                    movefile(specList.paths.templates,...
                                             [pwd,'/previousTemplates_',   regexprep(   char(datetime(now,'ConvertFrom','datenum')), '\W','-')]);
...
%             
%             % Check to see if a .com file exists in the pipe templates directory
%             
%             % Give option to pick a .com file or generate de novo using basicFT1.com
              
                  [file,path,cancel] = ft_comTemplate(specList,repSpecName);                  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      %% Make and run the ft_com files
            ...
            [output] = makeAndRunFT_comFiles('generateFTcoms.sh','runFTfiles.com',specList,'batchMode');
